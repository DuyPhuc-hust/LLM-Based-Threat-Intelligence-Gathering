...
class SQLiteStore:
    ...
    def recent_documents_metadata(self, days: int = 7, limit: int = 100000) -> list[sqlite3.Row]:
        with self.connect() as connection:
            return list(
                connection.execute(
                    """
                    SELECT id, source_id, source_name, source_type, language, published_at, collected_at
                    FROM documents
                    WHERE datetime(COALESCE(published_at, collected_at)) >= datetime('now', ?)
                    ORDER BY COALESCE(published_at, collected_at) DESC
                    LIMIT ?
    ...
    def all_entities(self, limit: int = 20000) -> list[sqlite3.Row]:
        with self.connect() as connection:
            return list(
                connection.execute(
                    """
